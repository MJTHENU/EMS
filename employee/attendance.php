    ...
    <div class="att">
        ...
        <h1>Attendance System</h1>
            ...
            <form method="post" action="">
                ...
                <input type="hidden" id="check_in" value="<?php echo $check_in_time; ?>" />
                <input type="hidden" class="input--style-1" name="total_hours" id="total_hours" readonly>

                <button type="submit" name="update" id="logoutButton">Logout</button>
            </form>
...
$stmtAttendance->close();
    ...
    if (!checkInTime || !checkOutTime) {
        alert("Check-in or Check-out time is missing.");
    ...
    const diffHours = (endTime - startTime) / (1000 * 60 * 60); // Convert milliseconds to hours

    if (diffHours <= 0.5) {
        alert("Contact HR/Admin");
        event.preventDefault(); // Prevent form submission
    } else if (diffHours < 7) {
        alert("Half Day Leave");
    } else if (diffHours < 8) {
        alert("Full Day Leave");
    } else if (diffHours >= 8) {
        // Form submission allowed
        return true;
    } else {
        alert("Error in time calculation. Please contact HR/Admin.");
        event.preventDefault(); // Prevent form submission
    }
});
