...
// Query to fetch employee details for the table with type = 3
$sql = "SELECT 
            employee.type, 
            employee.emp_id, 
            employee.img, 
        ...
        WHERE employee.type = 3"; // Filter by type = 3
...
$sql_employee_options = "SELECT emp_id, first_name FROM employee WHERE type = 3";
...
<h2 class="h2">Trainee Details</h2>
<div>
    <div class="input-container">
        <input list="employee-options" id="employee-input" placeholder="Search Emp ID / Name">
        <i class="fa fa-search search-icon"></i>
        <datalist id="employee-options">
            <?php while ($row = mysqli_fetch_assoc($employee_options_result)): ?>
                <option value="<?php echo $row['emp_id'] . ' - ' . $row['first_name']; ?>"></option>
            <?php endwhile; ?>
        </datalist>
    </div>
    <button class="add-emp">
        <a href="add-employee.php?type=trainee">
            <i class="fa-solid fa-plus"></i> Add Trainee
        </a>
    </button>
</div>
    ...
    <table id="employee-table">
        ...
        <?php
            ...
            while ($employee = mysqli_fetch_assoc($result)) {
                ...
                // echo "<td class='emp-id'>".$employee['type']."</td>";
                echo "<td class='emp-id'>";
                if ($employee['type'] == 3) {
                    echo "Trainee";
                } else {
                    echo $employee['type']; // You can add more conditions for other types if needed
                }
                echo "</td>";
